...
function Errors(errors: string[]): JSX.Element[] {
  ...
  for (let i = 0; i < errors.length; i++) {
    rows.push(<p className="block" key={i}>{ errors[i] }</p>);
  }
  return rows;
}
...
<div className="grid w-full h-screen place-content-center">
		<form className="w-full max-w-sm" action={action}>
			...
			<div className="md:flex md:items-center mb-1">
				<div className="md:w-1/3">
					<label className="block font-bold md:text-right mb-1 md:mb-0 pr-4" htmlFor="inline-game-id">
						Steam Game ID
					</label>
				</div>
				<div className="md:w-2/3">
			...
			<div className="md:flex md:items-center mb-6">
				...
				<span className="md:w-2/3 flex flex-col">
					<span>https://store.steampowered.com/app/<b className="text-primary">730</b>/CounterStrike_2/</span>
				</span>
			</div>
			<div className="md:flex md:items-center">
				<div className="md:w-1/3"></div>
				<div className="md:w-2/3">
					<button className="border-text border-2 hover:text-primary hover:border-primary focus:bg-primary focus:text-background focus:border-primary font-bold py-2 px-4 rounded" type="submit">
						Join Game
					</button>
				</div>
			</div>
		</form>
		<div id="indicator" className={"md:items-center mt-6 " + (pending ? "block" : "hidden")}>
			<div className="md:w-1/3"></div>
